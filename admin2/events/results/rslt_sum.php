...
$club = "None"; # overwrite below for 2013+

if ($year >= 2013) {
	$db = new MeekroDB();
	//$sql = "SELECT name,rank_score_top as score,sex FROM result_runner WHERE id = %i";
	$sql  = "SELECT r.name,r.sex,(SELECT k.rank_score FROM runner_rank k WHERE k.rank_year = %i AND k.runner_id = r.id AND k.crs_id = 0) as score, r.club_name";
	$sql .= " FROM result_runner r";
	$sql .= " WHERE r.id = %i";
	list($name,$sex,$score,$club) = $db->queryFirstList($sql,$year,$runner_id);
	if (($score == NULL) || ($score == ""))
		$score = 0;
} else {
		...
		if ($num_events) {
			...
			foreach ($rs as $row) {
				if ($year >= 2013) {
					...
					$yourPace[$indx]= getYourPace($db,$runner_id,$row['course_id'],$row['yourtime'],$year2);
				}
				$indx++;
			}
